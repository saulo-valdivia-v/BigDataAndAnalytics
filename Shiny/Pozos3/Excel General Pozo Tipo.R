...
names(prod)[names(prod) == "�..Anio"] <- "Anio"
...
# Se genera el dataframe de produccion por año
anual <- prod %>%
  select(Anio, Oil, Ivy) %>%
  group_by(Anio) %>%
  summarise_all(sum)
...
# Se cambia el nombre a la primera columa
names(precios)[names(precios) == "�..mes"] <- "Anio"
