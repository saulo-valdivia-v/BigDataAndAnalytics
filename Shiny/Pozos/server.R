...
shinyServer(function(input, output,session) {
  ...
  getCash <- reactive({
    #### OPEX Y CAPEX ####
    if (input$tabset1 == 'Oil'){
        lifting_cost_BOE <- as.numeric(input$liftingBoe)
        lifting_cost_MMBTU <- lifting_cost_BOE/6
    }
    
    if (input$tabset1 == 'Gas'){
      lifting_cost_MMBTU <- as.numeric(input$liftingMMTU)
      lifting_cost_BOE <- lifting_cost_MMBTU*6
    }
    
    #lifting_cost_MMBTU <- 0.5
    #lifting_cost_BOE <- lifting_cost_MMBTU*6
    capex <- as.numeric(input$capex)
    ...
    regalias <- as.numeric(input$regalia)
    ...
    # IIGG para el aÃÂ±o 2020
    IIGG_2020 <- as.numeric(input$IIGG_2020)
    ...
    tasa_desc <- as.numeric(input$desc)
  ...
  output$acumulada <- DT::renderDataTable({
    ...
    if(!is.na(as.numeric(input$regalia))) {
      payOutTable <- getPayout()
      payOutTable 
    }
    },
    options = list(
  ...
  IAI <- reactive({
    cashIai <- getCash()
    td <- as.numeric(input$desc)
    c <- npv(td, cashIai$CAPEX_MMBTU)*-1
  }) 
